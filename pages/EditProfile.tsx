...
export const EditProfile: React.FC = () => {
  const { profile, refreshData, goBack } = useApp();
  ...
  const handleSave = async (e: React.FormEvent) => {
      ...
      try {
          ...
          setMessage({ type: 'success', text: 'Profile updated successfully!' }); await refreshData(); setTimeout(() => { goBack(); }, 1500);
  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-fade-in pb-24 md:pb-8 max-w-2xl mx-auto">
        <div className="flex items-center gap-4 mb-4 md:mb-6"><button onClick={goBack} className="p-2 bg-surface rounded-xl border border-border text-secondary hover:text-primary transition-colors"><ArrowLeft size={20} /></button><h2 className="text-xl md:text-2xl font-bold text-primary">Edit Profile</h2></div>
